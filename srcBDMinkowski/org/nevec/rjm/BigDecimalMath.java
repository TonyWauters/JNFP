...
{
	public static BigDecimal TWO = new BigDecimal(2, MathContext.DECIMAL128).setScale(128);
...
//                }
        ...
        public static BigDecimal atan2(BigDecimal numerator, BigDecimal denominator) {
			if(denominator.compareTo(BigDecimal.ZERO)==0){
				if(numerator.compareTo(BigDecimal.ZERO)>=0){
					return pi(MathContext.UNLIMITED).divide(TWO, MathContext.UNLIMITED);
				}
				else return pi(MathContext.UNLIMITED).divide(TWO, MathContext.UNLIMITED).negate();
			}
			else{
				return atan(numerator.divide(denominator, MathContext.UNLIMITED));
			}
		}
